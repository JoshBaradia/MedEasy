...
class KNN:
    ...
    def find_distance(self, x):
        ...
        for point in x:
            ...
            for point1 in self.data:
                ...
                difference = abs(point - point1)
                difference = difference ** self.p
                dis = 0

                for cor in difference:
    ...
    def k_neighbours(self, x):
        ...
        """
        # TODO
        p_distance = self.find_distance(x)
        n_distance = []
        idx_of_neigh = []

        for point in p_distance:
            ...
            n_distance.append(temp_dist)
            idx_of_neigh.append(temp_idx)

        return n_distance,idx_of_neigh
    ...
    def predict(self, x):
        ...
        n_distance,idx_of_neigh = self.k_neighbours(x)
        pred = []
        eps = 1e-9
            ...
            for ind in range(0,len(indices)):
                ...
                if(self.weighted):
                    ...
                    if(self.target[indices[ind]] not in d):
                        ...
                        d[self.target[indices[ind]]] = 1/(n_distance[i][ind] + eps)
